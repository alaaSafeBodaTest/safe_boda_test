...
class LoadUserFollowers @Inject constructor(private val repo: IUserDetailsRepository): IUseCaseTemplate<LoadUserFollowRequestEntity, List<User>, IFailure> {

    override suspend fun runAsync(params: LoadUserFollowRequestEntity): Either<IFailure?, List<User>> {
        return repo.loadUserFollowersByUsername(params)
    }
}